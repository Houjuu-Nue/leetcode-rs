
use leetcode_rs::p00xx::p28::*;

#[derive(Debug, Clone)]
struct TestCase {
    input : Input,
    answer: Output,
}

#[test]
fn t28() {

    let solutions = [
        Box::new(Solution0) as Box<dyn Solution>,
        // Box::new(Solution1) as Box<dyn Solution>,
    ];
    
    let test_cases = [
        TestCase {
            input: Input {
                haystack: String::from("hello"),
                needle  : String::from("ll"),
            },
            answer: 2,
        },
        TestCase {
            input: Input {
                haystack: String::from("aaaaa"),
                needle  : String::from("bba"),
            },
            answer: -1,
        },
    ];

    for (i, solution) in solutions.into_iter().enumerate() {

        for (j, test_case) in test_cases.iter().cloned().enumerate() {

            let test_answer = solution.str_str(test_case.input.haystack, test_case.input.needle);

            assert_eq!(test_answer, test_case.answer,
                "Test failed on Solution {} TestCase {}.\n\
                Correct  answer is {}\n\
                TestCase answer is {}\n",
                i, j, &test_case.answer, &test_answer);
        }
    }
}

